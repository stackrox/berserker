...
#   * Create and start up a new tap device for berserker to use
...
while getopts ":a:t:uio" opt; do
  case $opt in
    ...
    ;;
    o) CONFIGURE_TAP_IF_EXISTS="true"
    ;;
...
then
    ...
    if [[ "${CONFIGURE_TAP_IF_EXISTS}" != "true" ]]
    then
        exit 1;
    fi
fi

echo "Creating tap device ${NAME} for user ${USER}..."
...
then
    ...
    # Excempt tap device from potentiall reject all rule
